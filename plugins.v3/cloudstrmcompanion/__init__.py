...
from app.schemas.types import EventType, NotificationType, MediaType, MediaSource
        ...
        for medis_title_year_season in list(self._medias.keys()):
            ...
            if (datetime.now() - last_update_time).total_seconds() > int(self._interval) \
                ...
                if self._notify:
                    ...
                    # 获取封面图片
                    _tmdbid = getattr(file_meta, "tmdbid", None)
                    mediainfo: MediaInfo = self.chain.recognize_media(
                        meta=file_meta,
                        mtype=media_type,
                        media_source=MediaSource.TMDB if _tmdbid else None,
                        media_id=str(_tmdbid) if _tmdbid else None)
